...
RSpec.describe Gamechanger::Commands::Base do
  ...
  let(:test_command_class) do
    Class.new(described_class) do
      ...
      # Expose privates for direct testing.
  ...
  describe '#run_command' do
    it 'returns the block result when no exception is raised' do
      result = command.run_command { 42 }
      expect(result).to eq(42)
    end

    it 'rescues AuthError, prints red, and exits 2' do
      expect {
    ...
    it 'rescues NetworkError, prints red, and exits 3' do
      expect {
    ...
    it 'rescues ConfigError, prints red, and exits 4' do
      expect {
        command.run_command { raise Gamechanger::ConfigError, 'no team_id' }
      }.to raise_error(SystemExit) { |e| expect(e.status).to eq(4) }
      expect(shell).to have_received(:say).with('Configuration error: no team_id', :red)
    end

    it 'rescues APIShapeError, prints red + yellow hint, and exits 3' do
      expect {
        command.run_command { raise Gamechanger::APIShapeError, 'unexpected shape' }
      }.to raise_error(SystemExit) { |e| expect(e.status).to eq(3) }
      expect(shell).to have_received(:say).with('Gamechanger API returned an unexpected format: unexpected shape', :red)
      expect(shell).to have_received(:say).with('The API may have changed. Check docs/research/gc-api-notes.md', :yellow)
    end

    it 'rescues StorageError, prints red + yellow recovery hint pointing at `refresh`, and exits 1' do
      expect {
        command.run_command { raise Gamechanger::StorageError, 'db locked' }
      }.to raise_error(SystemExit) { |e| expect(e.status).to eq(1) }
      expect(shell).to have_received(:say).with('Cache read failed — db locked', :red)
  ...
  describe '#build_formatter' do
    ...
    it 'returns a Table formatter for format=table' do
    ...
    it 'returns a Table formatter when format is missing or unknown' do
  ...
  describe '#load_config!' do
    ...
    it 'prints red and exits 4 when not configured' do
      allow(Gamechanger::Config).to receive(:new).and_return(
  ...
  describe '#resolve_target' do
    ...
    it 'prints red and exits 1 for an unparseable date string' do
      expect {
    ...
    it 'prints yellow and exits 1 when no date and no next game exists' do
