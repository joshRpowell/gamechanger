...
RSpec.describe Gamechanger::Commands::Plan do
  ...
  describe '#call with --games' do
    ...
    it 'exits 1 with red error on malformed --games date' do
      command = described_class.new(options: { games: 'bad-date' }, shell: shell)
  ...
  describe '#call with --from' do
    ...
    it 'defaults --to to --from when --to is omitted' do
      command = described_class.new(options: { from: '2026-03-21' }, shell: shell)
      allow(storage).to receive(:scheduled_games_between).and_return([])
    ...
    it 'exits 1 with red error on malformed --from date' do
      command = described_class.new(options: { from: 'bad' }, shell: shell)

      expect { command.call }.to raise_error(SystemExit) { |e| expect(e.status).to eq(1) }
      expect(shell).to have_received(:say).with(/Invalid --from date 'bad'/, :red)
    end

    it 'exits 1 with red error on malformed --to date' do
      command = described_class.new(options: { from: '2026-03-21', to: 'bad' }, shell: shell)

      expect { command.call }.to raise_error(SystemExit) { |e| expect(e.status).to eq(1) }
      expect(shell).to have_received(:say).with(/Invalid --to date 'bad'/, :red)
    end
  end
