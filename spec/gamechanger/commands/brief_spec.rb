...
RSpec.describe Gamechanger::Commands::Brief do
  ...
  describe '#call' do
    it 'aggregates availability, lineup, arcs, and equity for the resolved target date' do
      ...
      expect(storage).to have_received(:pitcher_availability_data).with(before_date: target)
      expect(storage).to have_received(:batter_lineup_data).with(before_date: target)
    end
    ...
    it 'maps APIShapeError to red + yellow + exit 3' do
      ...
      expect(shell).to have_received(:say).with(/Gamechanger API returned an unexpected format/, :red)
    end
  end
end
