...
module Gamechanger
  module Commands
    ...
    #
    # Subclasses implement #call. Subclasses wrap their work in #run_command,
    # which applies the 5-exception rescue chain that previously lived in
    # every Thor command method in cli.rb.
    #
    # Usage:
    ...
    class Base
      ...
      # Wraps a block in the standard 5-exception rescue chain.
      # Maps each exception class to (color, message, hint?, exit_code).
      def run_command
        yield
      rescue AuthError => e
        shell.say "Authentication error: #{e.message}", :red
        exit 2
      rescue NetworkError => e
        shell.say "Network error: #{e.message}", :red
        exit 3
      rescue ConfigError => e
        shell.say "Configuration error: #{e.message}", :red
        exit 4
      rescue APIShapeError => e
        shell.say "Gamechanger API returned an unexpected format: #{e.message}", :red
        shell.say 'The API may have changed. Check docs/research/gc-api-notes.md', :yellow
        exit 3
      rescue StorageError => e
        shell.say "Cache read failed — #{e.message}", :red
        shell.say 'Try deleting ~/.gamechanger/cache.db and re-running `gamechanger refresh`', :yellow
        exit 1
      end
      ...
      def load_config!
        ...
        unless config.configured?
          shell.say 'Not configured. Run `gamechanger setup` first.', :red
          exit 4
        end
        config
      end
