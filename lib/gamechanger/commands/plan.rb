...
module Gamechanger
  module Commands
    ...
    class Plan < Base
      ...
      # Resolve the list of game slots from --games, --from/--to, or next scheduled game.
        ...
        if options[:games]
          ...
          dates.map do |d|
            parsed = begin
              ...
              shell.say "Invalid date '#{d}' in --games — expected YYYY-MM-DD", :red
              exit 1
            end
      ...
      def parse_required_date!(value, flag_name)
        ...
        shell.say "Invalid #{flag_name} date '#{value}' — expected YYYY-MM-DD", :red
        exit 1
      end
    end
  end
