    ...
    class Markdown
      ...
      def lineup(target_date, game_info, optimizer)
        ...
        if optimizer.ranked.empty? && optimizer.unranked.empty?
          lines << "_No batting data in cache. Run `gamechanger refresh` to sync._"
          lines << ""
          return lines.join("\n")
        end
      ...
      def equity(rows)
        return "_No player data found. Run `gamechanger refresh` to sync._" if rows.empty?
      ...
      def progress(arcs)
        return "_No player data cached. Run `gamechanger refresh` to sync._" if arcs.empty?
